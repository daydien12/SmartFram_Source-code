...
void SmartFram_SYS_ReadAllSensor()
{
	...
	MAX44009_Init(&hi2c3);
	HAL_Delay(100);
	MAX44009_ReadLightHighResolution(&Sys_DataSensorRead.Data_Senser_Lux);
...
void Smartfram_SYS_DebugSensor(uint8_t mode)
{
  ...
  if (mode == 1)
  {
		sprintf(buffer_temp, "%.2f", Sys_DataSensorRead.Data_Senser_Lux);
		APP_PPRINTF("MAX44009 Lux: %s\n\r", buffer_temp);  
...
void InitValue(void)
{
  ...
  Sys_DataSensorRead.Data_Senser_Lux = 0;
