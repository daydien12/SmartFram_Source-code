...
**/
void smartfram_IncrementIdMsg(uint32_t dataCo2)
{
	FS_FlashDevice_ReadMesseger();
	Sys_DeviceFlashData_MSG.ID_Msg++;
	Sys_DeviceFlashData_MSG.CO2 = dataCo2;
	Sys_DeviceFlashData_MSG.ERROR = vr_Error;
	FS_FlashDevice_WriteMesseger();
...
void FS_FlashUpdate_All(void)
{
	...
	vr_Error = Sys_DeviceFlashData_MSG.ERROR;
}
...
void FS_FlashDevice_Debug(void)
{
	...
	sprintf(buffer_temp, "%d",Sys_DeviceFlashData_MSG.CO2);
	APP_PPRINTF("CO2: %s \n"	,buffer_temp);
...
void FS_FlashDevice_ReadMesseger(void)
{
	...
	Sys_DeviceFlashData_MSG.CO2 = TempData->CO2;
