...
static void FS_ReadSensor(void)
{
	uint32_t Temp, Hum, SumTemp, SumHum;
	if(VarApp.FlagStartReadSensor == 1)
	{
		SmartFram_SYS_ReadAllSensor();
		if(((Sys_DataSensorRead.Data_Senser_Humidity == 0)||(Sys_DataSensorRead.Data_Senser_Temperature == 0)))
		{
			Temp = Sys_DataSensorRead.Data_Senser_Temperature;
			Hum = Sys_DataSensorRead.Data_Senser_Humidity;
			if(Temp > Sys_DeviceFlashData_MSG.Temperature)
			{
				SumTemp = Temp - Sys_DeviceFlashData_MSG.Temperature;
			}
			else
			{
				SumTemp = Sys_DeviceFlashData_MSG.Temperature - Temp;
			}
			
			if(Hum > Sys_DeviceFlashData_MSG.Humidity)
			{
				SumHum = Hum - Sys_DeviceFlashData_MSG.Humidity;
			}
			else
			{
				SumHum = Sys_DeviceFlashData_MSG.Humidity - Hum;
			}
