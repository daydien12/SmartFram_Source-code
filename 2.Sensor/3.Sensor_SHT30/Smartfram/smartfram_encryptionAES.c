...
__ALIGN_BEGIN uint32_t pLocalDeviceKeyAES[4] __ALIGN_END      = {0x00000000, 0x00000000, 0x00000000, 0x00000000};
__ALIGN_BEGIN uint32_t pNetKeyAES[4] __ALIGN_END         = {0x00000000, 0x00000000, 0x00000000, 0x00000000};
...
/**
	Mo ta: Ham ma hoa su dung NetKey
	Tham so:	
		-InputData: Mang(uint8_t) data can ma hoa 
...
/**
	Mo ta: Ham giai ma su dung NetKey
	Tham so:	
		-InputData: Mang(uint8_t) data can giai ma
		-Key: DeviceKey ma hoa tuong ung
		-SizeDataIn: Size mang dau vao(de tinh toan block)
...
void Smartfram_Encrypted(encrypted_t* data)
{
  ...
  switch (data->TypePayload)
  {
    ...
    case CONFIRM:
      ...
      SizeArray = Smartfram_EncryptedPublic(TempData1, pNetKeyAES, data->SizeDataIn, SM_AES_CONFIRM_DF, TempData);
      data->SizeDataOut = Smartfram_EncryptedPrivate(TempData, SizeArray, data->EncryptedOut);
      break;

    case DEVICETOGETWAY:
      CreateCheckSum(data->EncryptedIn, TempData1, data->TypePayload);
      SizeArray = Smartfram_EncryptedPublic(TempData1, pNetKeyAES, data->SizeDataIn, SM_AES_MSGDEVICEGETGAY_DF, TempData);
      data->SizeDataOut = Smartfram_EncryptedPrivate(TempData, SizeArray, data->EncryptedOut);
      break;
...
void Smartfram_Decrypted(decrypted_t* data)
{
  ...
  switch (data->TypePayload)
  {
    ...
    case COMPLETED:
      ...
      {
        Smartfram_DecryptedPublic(TempData, pLocalDeviceKeyAES, data->SizeTypeOut , SM_AES_COMPLETED_DF, data->DecryptedOut);
    ...
    case GETWAYTODEVICE:
      ...
      if (CheckDecrypted(TempData, data->TypePayload, &AdressUnicast, 1) == 1)
      {
        Smartfram_DecryptedPublic(TempData, pLocalDeviceKeyAES, data->SizeTypeOut , SM_AES_MSGDEVICEGETGAY_DF, data->DecryptedOut);
        if (CheckDecrypted(data->DecryptedOut, data->TypePayload, &AdressUnicast, 0))
        {
          data->Check = 1;
        }
...
static uint8_t Check_Header_TypeMsg(uint16_t HeaderIn, uint16_t HeaderCheck, uint8_t TypeMsgIn, uint8_t TypeMsgCheck)
{
	#if BUG_ENCRY_CHECK_HEADER_TYPEMSG
