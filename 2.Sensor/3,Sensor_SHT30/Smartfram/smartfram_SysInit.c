...
{
	smartfram_GPIO_WakeupReadSensor(1);
	HAL_Delay(100);
	MX_I2C3_Init();
	HAL_Delay(100);
	SmartFram_SHT30_Init(&hi2c3);
	HAL_Delay(100);
	Sys_DataSensorRead.Data_Senser_Temperature = SmartFram_SHT30_ReadTemperature();
	Sys_DataSensorRead.Data_Senser_Humidity = SmartFram_SHT30_ReadHumidity();
	smartfram_GPIO_WakeupReadSensor(0);
...
void Smartfram_SYS_DebugSensor(uint8_t mode)
{
  ...
  if (mode == 1)
  {
		sprintf(buffer_temp, "%.2f", Sys_DataSensorRead.Data_Senser_Temperature);
		APP_PPRINTF("Temp: %s ", buffer_temp);
		sprintf(buffer_temp, "%.2f", Sys_DataSensorRead.Data_Senser_Humidity);
		APP_PPRINTF("Hum: %s \n\r", buffer_temp);
...
void InitValue(void)
{
  ...
  Sys_DataSensorRead.Data_Senser_Humidity = 0;
